...
class HandDetector:
    ...
    def get_landmarks_3d(self, hand_landmarks, image_width):
        ...
        Args:
            hand_landmarks: MediaPipe 손 랜드마크
            image_width: 이미지 너비 (Z 좌표 스케일링)
        
        Returns:
            dict: {landmark_name: z_value}
        """
        lm = hand_landmarks.landmark
        
        def get_z(landmark_enum):
            """랜드마크 enum → Z 좌표"""
            return float(lm[landmark_enum].z) * image_width
        
        return {
            'idx_tip_z': get_z(mp.solutions.hands.HandLandmark.INDEX_FINGER_TIP),
            'thm_tip_z': get_z(mp.solutions.hands.HandLandmark.THUMB_TIP)
        }
    ...
    def get_normalized_landmarks(self, hand_landmarks):
        ...
        Returns:
            dict: {landmark_name: (norm_x, norm_y)}
        """
        lm = hand_landmarks.landmark
        
        return {
            'idx_tip': (lm[mp.solutions.hands.HandLandmark.INDEX_FINGER_TIP].x,
